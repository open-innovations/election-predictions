...
export default (data: Record<string, unknown>[]) => {
  ...
  return data.map(c => {
    ...
    const resultTable = sortedResults.map(x =>
      `<tr><td scope="row">${ parties[x[0]] }</td><td>${ (x[1] * 100).toFixed(1) }%</td></tr>`
    ).join('')

    return {
      ...c,
      tooltip: `<strong class="title">${c.pcon_nm}</strong><p>Winner: ${ parties[c.winner] }</p><table><thead><tr><th scope="column">Party</th><th scope="column">Prediction</th></tr></thead><tbody>${ resultTable }</tbody></table>`
    }
  });
}